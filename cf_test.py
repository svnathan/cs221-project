...
def main():
	tags = stem_tags.get_features() #return list of tags
	#tags = kmeans_tags.get_u_features() #return list of tags
	#tags = stem_tags.get_q_features() #return list of tags
	...
	for i in range(100):
		...
		results.append(recommendation(q[i], rndm_users[i], questions, answers, votes, counts, normalized_reps))
		#print rndm_users[i]
...
def recommendation(q, users, questions, answers, votes, counts, rep):
	recs = {}
	rec_sum = 0
	for u in users:
		if u not in answers: 
